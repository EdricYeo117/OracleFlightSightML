...
class EyeGazeEstimator:
    def __init__(
        self,
        center_tolerance_x: float = 0.06,
        center_tolerance_y: float = 0.08,
        ...
        self.baseline_alpha = float(np.clip(baseline_alpha, 0.0, 1.0))

        self._last_eye_dx = None
        self._last_eye_dy = None

        self.baseline_eye_x = 0.5
        self.baseline_eye_y = 0.5
    ...
    def set_baseline(self, eye_x: float, eye_y: float):
        self.baseline_eye_x = float(eye_x)
        self.baseline_eye_y = float(eye_y)
        self.is_calibrated = True

    def update_baseline(self, eye_x: float, eye_y: float):
        a = self.baseline_alpha
        self.baseline_eye_x = (1.0 - a) * self.baseline_eye_x + a * float(eye_x)
        self.baseline_eye_y = (1.0 - a) * self.baseline_eye_y + a * float(eye_y)
        self.is_calibrated = True
    ...
    def estimate(self, points_2d: np.ndarray):
        ...
        eye_dx_raw = eye_x - self.baseline_eye_x
        eye_dy_raw = eye_y - self.baseline_eye_y

        eye_dx, eye_dy = self._smooth(eye_dx_raw, eye_dy_raw)
        ...
        return {
            ...
            "eye_x": float(eye_x),
            "eye_y": float(eye_y),
            "eye_dx_raw": float(eye_dx_raw),
            "eye_dy_raw": float(eye_dy_raw),
            "eye_dx": float(eye_dx),
            "eye_dy": float(eye_dy),
            "direction": direction,
            "confidence": float(confidence),
            "blink_like": bool(blink_like),
            "baseline_eye_x": float(self.baseline_eye_x),
            "baseline_eye_y": float(self.baseline_eye_y),
